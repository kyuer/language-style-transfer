...
from options_cn import load_arguments
from file_io import load_sent_cn, write_sent
from utils import *
from nn import *
...
class Model(object):
    ...
    def __init__(self, args, vocab):
        ...
        half = tf.cast(self.batch_size // 2, tf.int32)  #tf.cast() to int32
...
if __name__ == '__main__':
    ...
    #####   data preparation   #####
    if args.train:
        train0 = load_sent_cn(args.train + '.0', args.max_train_size)
        train1 = load_sent_cn(args.train + '.1', args.max_train_size)
    ...
    if args.dev:
        dev0 = load_sent_cn(args.dev + '.0')
        dev1 = load_sent_cn(args.dev + '.1')

    if args.test:
        test0 = load_sent_cn(args.test + '.0')
        test1 = load_sent_cn(args.test + '.1')
